  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (message.toLowerCase().includes('email not confirmed') || message.toLowerCase().includes('confirm')) {
        ...
        setError('Email request rate exceeded. Wait 2 minutes before trying again, then check your inbox rather than resending immediately.');
        setResendCooldown(120);
      } else if (message.toLowerCase().includes('duplicate') || message.toLowerCase().includes('already registered')) {
        setError('An account already exists with this email. If you already signed up, switch to login or reset your password.');
        ...
        setError('Invalid login credentials. Make sure your email and password are correct, and that your email has been confirmed.');
      } else {
        setError(message);
      }
    } finally {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-8 shadow-lg">
            ...
            <div className="mb-4 rounded bg-red-100 p-3 text-red-700 text-sm">
              ...
              {error.toLowerCase().includes('invalid login credentials') && (
                <p className="mt-2 text-xs text-red-700/80">
                  If your email has been confirmed, refresh the login page and try again. Verify your email/password spelling or reset your password if needed.
                </p>
              )}
            </div>
          )}
          ...
          {lastSignupEmail && !isSignup && (
            <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-slate-800">
              ...
              <p className="mt-1 text-sm text-slate-600">
                Email resending is limited by Supabase. Wait {formatCooldown(resendCooldown)} before requesting another confirmation email.
              </p>
              <button
                type="button"
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading || (isSignup && !signupPasswordValid)}
            ...
            {isSignup && !signupPasswordValid && (
              <p className="mt-3 text-sm text-slate-600">
                Complete all password requirements before signing up to avoid repeated failures.
              </p>
            )}
          </form>
